...
chrome.webNavigation.onCommitted.addListener(async (details) => {
  ...
  try {
    ...
    if (hostname.endsWith('.trycloudflare.net') || hostname === 'trycloudflare.net') {
      chrome.tabs.update(details.tabId, {
        url: chrome.runtime.getURL('blocked.html') + `?url=${encodeURIComponent(url)}&reason=cloudflare_tunnel`
      });
      return;
    }
    ...
    const p = await predictProbability(features, url);
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.action === 'getRiskForUrl') {
    try {
      ...
      try {
        const features = extractFeaturesFromUrl(url);
        if (features) p = predictProbability(features, url);
      } catch (e) {
